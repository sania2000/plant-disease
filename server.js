...
const API_URL = 'https://my-api.plantnet.org/v2/identify/all?api-key=2b10UwYbG9xqqF6qnzZpgVYe5u';
...
axios.post('https://api.plant.id/v2/identify', data).then(res => {
    setTimeout(async function () {
        console.log("succes! is_healthy_probability:", res.data.health_assessment.is_healthy_probability)


        var newArray = res.data.health_assessment.diseases.filter(function (el)
{
